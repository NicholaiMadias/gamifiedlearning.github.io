...
/* ── Demo / localStorage database shim ─────────────────────
   Provides a Firebase-compatible db.ref().get() / .set() / .update() API
   backed by localStorage so the dashboard can be exercised without
   a live backend.
   ─────────────────────────────────────────────────────────── */
  ...
  ref(path) {
    return {
      ...
      async update(updates) {
        ...
        if (!trimmed) {
          // Root-level multi-path update — keys are full paths such as
          // "users/uid/tools/networkDefense".  Walk each path and set the
          // leaf value so all tool flags land in a single localStorage write.
          const users = getDemoUsers();
          for (const [fullPath, value] of Object.entries(updates)) {
            const segs = fullPath.split('/');
            if (segs[0] === 'users' && segs[1]) {
              users[segs[1]] = users[segs[1]] || {};
              let obj = users[segs[1]];
          ...
          // Scoped update — shallow-merge into an existing user record
