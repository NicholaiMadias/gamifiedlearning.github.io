...
export async function renderToolAssignment(db) {
  ...
  saveBtn.onclick = async () => {
    ...
    try {
      ...
      // Group checkboxes by uid, then issue one multi-path update per user.
      // This replaces N sequential (or N concurrent) single-key writes with
      // a single atomic update per user — far fewer round-trips and no risk
      // of hitting backend rate limits.
      const toolsByUid = {};
      boxes.forEach(box => {
        toolsByUid[box.dataset.uid] = toolsByUid[box.dataset.uid] || {};
        toolsByUid[box.dataset.uid][box.dataset.tool] = box.checked;
      });
      await Promise.all(
        Object.entries(toolsByUid).map(([uid, toolMap]) => setUserTools(db, uid, toolMap))
      );
