...
/**
 * Public site paths that do not require authentication or a role check.
 * Requests matching these paths are allowed through without calling requireRole().
 * @type {string[]}
 */
...
export function requireRole(allowedRoles) {
  // Public paths bypass authentication entirely — no redirect, no role check.
  const currentPath = window.location.pathname.replace(/\/+$/, '') || '/';
  if (PUBLIC_PATHS.includes(currentPath) || PUBLIC_PATHS.includes(currentPath + '.html')) {
