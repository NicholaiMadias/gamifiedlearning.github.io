...
- **Deployment:** GitHub Pages to **[matrix.amazinggracehl.org](https://matrix.amazinggracehl.org)**
...
The site deploys automatically to **[matrix.amazinggracehl.org](https://matrix.amazinggracehl.org)** via GitHub Actions whenever code is pushed to the `main` branch.
...
The `CNAME` file contains `matrix.amazinggracehl.org`. The `validate-cname.yml` workflow validates that it matches the expected domain on pull requests.
